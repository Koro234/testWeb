...
function getDayOnDesc(date) {
    var tempDay;
    for (var i = 0; i < dateArray.length; i++)
    {
        if (dateArray[i].getFullYear() == date.getFullYear()
        && dateArray[i].getMonth() == date.getMonth()
        && dateArray[i].getDate() == date.getDate())
        {
            tempDay = i;
        }
    }
...
function CreateTaskOnDesk(User, Task) {
    ...
    var startDay = getDayOnDesc(startDate);
    var endDay = getDayOnDesc(endDate);
    if (startDate <= endDate) 
    {
        ...
        {
            var event = ConstructorTaskElement(Task, 1, 14);
            Task.htmlElement = event;
            User.htmlElement.lastChild.appendChild(event);
        }
    }
...
function UpdateDatesLine() {
    ...
    for (var i = 0; i < 14; i++)
    {
        ...
        tempDate = new Date(tempDate.setHours(4));
        if (i === 0)
        {
            leftDate = tempDate;
        }
...
// открытие приложения
async function Load() {
    users = await fetch('https://varankin_dev.elma365.ru/api/extensions/2a38760e-083a-4dd0-aebc-78b570bfd3c7/script/users')
        .then(response => {
            if (response.status >= 200 && response.status < 300)
            {
                return Promise.resolve(response);
    ...
    tasks = await fetch('https://varankin_dev.elma365.ru/api/extensions/2a38760e-083a-4dd0-aebc-78b570bfd3c7/script/tasks')
        .then(response => {
            if (response.status >= 200 && response.status < 300)
            {
                return Promise.resolve(response);
